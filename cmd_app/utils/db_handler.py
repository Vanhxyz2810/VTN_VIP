...
class DatabaseHandler:
    ...
    def get_current_bang_gia(self):
        """
        Lấy bảng giá hiện tại
        
        Returns:
            BangGia: Đối tượng bảng giá hiện tại hoặc None nếu không có
        """
        # Sử dụng phương thức get_bang_gia_hien_hanh đã được cập nhật
        return self.get_bang_gia_hien_hanh()
    
    def get_bang_gia_active(self):
        """
        Lấy bảng giá đang hoạt động (alias của get_bang_gia_hien_hanh)
        
        Returns:
            BangGia: Đối tượng bảng giá đang hoạt động hoặc None nếu không có
        """
        return self.get_bang_gia_hien_hanh()
